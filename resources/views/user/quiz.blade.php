...
.quizzes-container {
    display: grid;
    grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); /* Responsive grid with minimum card size */
    gap: 20px;
    padding: 10px 15px;
    width: 100%;
    max-width: 100%;
        ...
        @foreach ($courses as $course)
            <div class="quiz-card">
                <div class="quiz-card-header" 
                     style="background-image: url('{{ $course->course_image ? asset('storage/courses/images/' . $course->course_image) : 'https://via.placeholder.com/250x150' }}')">
                    <h3>{{ $course->title }}</h3>
                </div>
                <div class="quiz-card-body">
                    <p>{{ $course->description }}</p>
                    <div class="quiz-meta">
                        <span>{{ $course->questions->count() ?? 0 }} Questions</span>
                    </div>
                    @php
                    ...
                    @else
                        <button class="btn start-quiz-btn" 
                                ...
                                data-bs-target="#quizModal-{{ $course->id }}">
                            Start Quiz
                        </button>
                    @endif
                </div>
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            @if (!empty($course->questions) && $course->questions->count() > 0)
                                ...
                                <div id="message-container"></div> <!-- Success or error messages -->
                            @else
